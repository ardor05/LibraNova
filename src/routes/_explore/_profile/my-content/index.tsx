    ...
    <div className="w-full">
      <Button asChild>
        <Link to={'/my-content/new'}>
          <Plus size="18" />
          <span className="ml-1.5">New book</span>
        </Link>
      </Button>
      ...
      {isError && (
        <Alert variant="error" className="mt-8">
          <p>Oops, something went wrong while fetching your book.</p>
          <p className="mt-1 text-sm">{error.toString()}</p>
        </Alert>
      )}

      {isSuccess && protectedDatas.length === 0 && (
        <div className="mt-6 italic">You don't have any book yet.</div>
      )}

      {isSuccess && protectedDatas.length > 0 && (
        <>
          <div className="mt-9 text-xl">Last book</div>
          <div
